...
class PlacesService {
  ...
  Future<List<PlaceSearch>> getAutocomplete(String search) async {
    ...
    var url = Uri.parse(
        'https://maps.googleapis.com/maps/api/place/autocomplete/json?input=$search&key=$API_KEY&components=country:$country');
  ...
  Future<Place> getPlace(String placeId) async {
    ...
    var radius = '500';
    var url = Uri.parse(
        'https://maps.googleapis.com/maps/api/place/details/json?key=$API_KEY&place_id=$placeId');
    var response = await http.get(url);
  ...
  static Future<String> searchCoordinateAddress(
    ...
    //geolocator search position //assistant method
  ...
  static Future<DirectionDetails> obtainPlaceDirectionDetails(
    ...
    String directionsUrl =
        'https://maps.googleapis.com/maps/api/directions/json?origin=${initialPosition.latitude},${initialPosition.longitude}&destination=${finalPosition.latitude},${finalPosition.longitude}&mode=walking&key=$GoogleAPIKey';
    ...
    directionDetails.distanceValue =
        jsonResults[0]["legs"][0]["distance"]["value"];
    directionDetails.distanceText =
        jsonResults[0]["legs"][0]["distance"]["text"];

    directionDetails.durationValue =
        jsonResults[0]["legs"][0]["duration"]["value"];
    directionDetails.durationText =
        jsonResults[0]["legs"][0]["duration"]["text"];

    directionDetails.encodedPoints =
        jsonResults[0]["overview_polyline"]["points"];
  ...
  static Future<DirectionDetails> addingWaypoint(
      LatLng initialPosition, LatLng finalPosition, LatLng waypoint) async {
    //example https://maps.googleapis.com/maps/api/directions/json?
    // origin=Toronto&destination=Montreal
    // &key=YOUR_API_KEY

    String directionsUrl =
        'https://maps.googleapis.com/maps/api/directions/json?origin=${initialPosition.latitude},${initialPosition.longitude}&destination=${finalPosition.latitude},${finalPosition.longitude}&waypoints=${waypoint.latitude}%2C${waypoint.longitude}&mode=walking&key=$GoogleAPIKey';
